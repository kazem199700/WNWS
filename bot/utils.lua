...
tdcli_function ({
  ...
  if not lang then
    ...
    return tdcli.sendMessage(msg.to.id, msg.id, 0, "_تم تعيين جميع مديري المجموعة لمنصب المدير وتم تعيين المجموعة كمالك للمجموعة_", 0, "md")
     end
 end
...
local lang = redis:get(hash)
  ...
  if not lang then
    ...
    return 'لا يتم إضافة المجموعة إلى قائمة مجموعات إدارة الروبوت'
   end
  end
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
     if not lang then
					...
					return "*لم تتم إزالة أي مستخدمين من هذه المجموعة*"
              end
				end
       if not lang then
   message = '*List of banned users :*\n'
         else
   message = '_قائمة بالمستخدمين المحرومين من المجموعة :_\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
        if not lang then
					...
					return "*قائمة المستخدمين الصامتين فارغة*"
             end
				end
      if not lang then
   message = '*List of silent users :*\n'
       else
   message = '_قائمة المستخدمين الصامتين :_\n'
    end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['whitelist']) == nil then --fix way
     if not lang then
					...
					return "*لا يوجد مستخدمون في القائمة البيضاء*"
              end
				end
       if not lang then
   message = '*Users of white list :*\n'
         else
   message = '_المستخدمون في القائمة البيضاء :_\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data['gban_users']) == nil then --fix way
    if not lang then
					...
					return "*لم يتم تجريد أي مستخدم من مجموعات الروبوت*"
             end
				end
        if not lang then
   message = '*List of globally banned users :*\n'
   else
   message = '_قائمة بالمستخدمين المحرومين من مجموعات الروبوت :_\n'
   end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(msg.chat_id_)]['filterlist']) == nil then --fix way
    ...
    return "_قائمة الكلمات التي تمت تصفيتها فارغة_"
     end
  end
  if not data[tostring(msg.chat_id_)]['filterlist'] then
    data[tostring(msg.chat_id_)]['filterlist'] = {}
    save_data(_config.moderation.data, data)
      ...
      if not lang then
       ...
       filterlist = '_قائمة الكلمات التي تمت تصفيتها:_\n'
    end
 local i = 1
   for k,v in pairs(data[tostring(msg.chat_id_)]['filterlist']) do
